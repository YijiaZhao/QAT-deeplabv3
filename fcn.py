...
class FCNHead(nn.Sequential):
    def __init__(self, in_channels: int, channels: int, quantize: bool = False) -> None:
        inter_channels = in_channels // 4
        if quantize:
            layers = [
                quant_nn.QuantConv2d(in_channels, inter_channels, 3, padding=1, bias=False),
                nn.BatchNorm2d(inter_channels),
                nn.ReLU(),
                nn.Dropout(0.1),
                quant_nn.QuantConv2d(inter_channels, channels, 1),
            ]
        else:
            layers = [
